        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.dataIndex === 'name'">
              <span v-if="record.step4New" class="new-project-name"
                >{{ record.name }}
                <icon>
                  <template #component>
                    <svg
            ...
            <span :class="(record.step4New) ? 'new-project-name' : ''">{{
              record.number
            }}</span>
          </template>
            ...
            <template v-else-if="column.dataIndex === 'type'">
              <span :class="(record.step4New) ? 'new-project-name' : ''">{{ typeMap[record.type] }}</span>
            </template>
      ...
      return {
        onClick: (event) => {
          ...
          if (!!record.step4New) {
              ...
              record.step4New = !record.step4New;
              updateIsNewProject(record.processId, null, null, false, null)
                .then((response) => {
                  // fetchData("", "", "", "" + dayjs().year(), "", "");
                })
                .catch((err) => {
